...
FakeMain( const ros::NodeHandle& nh, int argc, char **argv )
	:run_status_(0), status_changed_(0), dvgrab(argc, argv)
{
	int ret = 0;
...
void control_cb(const ultraspeech::ControlConstPtr& msg)
{
	...
	run_status_ = msg->run;
	//subdir_ = (std::string)(msg->directory + "/ultrasound/");
	status_changed_ = 1;
	if (run_status_ == 1)
		dvgrab.startCapture();
	else 
...
{
	ros::init(argc, argv, "ultrasound_dvgrab", ros::init_options::AnonymousName);
	ros::NodeHandle nh;
