...
public class Binary extends Value {
	...
	public void merge(Value v) {
		if (!(v instanceof Binary))
			throw new RuntimeException("Cannot merge different types: Binary - " + 
	...
	public double distance(Value v) {
		...
		Binary b = (Binary) v;
		if (b._uniqueValue == _uniqueValue)
			return 0.0;
	...
	public double multiply(Value v) {
		if (!(v instanceof Binary))
			throw new RuntimeException("Cannot take dot of different types: Binary - " + 
					v.getClass().getSimpleName());
