...
public class Real extends Value {
	...
	public void merge(Value v) {
		if (!(v instanceof Real))
			throw new RuntimeException("Cannot merge different types: Real - " + 
	...
	public double distance(Value v) {
		if (!(v instanceof Real))
			throw new RuntimeException("Distance undefined between: Real - " + 
					v.getClass().getSimpleName());

		Real r = (Real) v;

		// Return the value that is the distance to the 
		// mean of all of the values that we have seen.
	...
	public double multiply(Value v) {
		if (!(v instanceof Real))
			throw new RuntimeException("dot undefined between: Real - " + 
					v.getClass().getSimpleName());
