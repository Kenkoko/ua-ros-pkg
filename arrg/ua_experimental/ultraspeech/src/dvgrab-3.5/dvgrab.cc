		...
		m_timeSplit(0), m_srt( false ), m_isNewFile(false)
{
	...
	control_sub_ = local_nh.subscribe("/control", 1, &DVgrab::control_cb, this);
...
void DVgrab::control_cb(const ultraspeech::ControlConstPtr& msg)
{
	subdir_ = (std::string)(msg->directory + "/ultrasound/ultrasound_");
	m_dst_file_name = convertStringToChar(subdir_);
}
...
void DVgrab::print_version()
{
	...
	cerr << "Modified version of 3.5 to run with ros" << endl;
}
...
void DVgrab::startCapture()
{
	...
	if ( !g_done && m_frame )
	{
		...
		sv_msg.filepath = "ros_dvgrab_start";
...
void DVgrab::stopCapture()
{
	...
	if ( m_writer != NULL )
	{
		...
		sv_msg.header.stamp = ros::Time::now();
		sv_msg.filepath = "ros_dvgrab_stop";
		savefile_pub_.publish(sv_msg);
