...
void IEEE1394Reader::StartROSFrames(){
  m_framenum = 0;
  m_publish = true;
}
void IEEE1394Reader::StopROSFrames(){
  m_publish = false;
}
...
{
	sendEvent( "iec61883Reader:: Destructor called" );
...
int iec61883Reader::Handler( unsigned char *data, int length, int dropped )
{
	...
	// ROS publishing of frame number, ROS messages get timestamped, so we can easily
	// get as close as possible to same timestamp in audio and other ROS datastreams.
	if ( m_publish == true ){
	  std_msgs::UInt64 msg;
	  msg.data = m_framenum++;
