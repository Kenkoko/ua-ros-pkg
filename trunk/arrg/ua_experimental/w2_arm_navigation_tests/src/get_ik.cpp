...
int main(int argc, char **argv){
  ...
  ros::service::waitForService("wubble_left_arm_kinematics/get_ik_solver_info");
  ros::service::waitForService("wubble_left_arm_kinematics/get_ik");

  ros::ServiceClient query_client = rh.serviceClient<kinematics_msgs::GetKinematicSolverInfo>("wubble_left_arm_kinematics/get_ik_solver_info");
  ros::ServiceClient ik_client = rh.serviceClient<kinematics_msgs::GetPositionIK>("wubble_left_arm_kinematics/get_ik");
  ...
  gpik_req.timeout = ros::Duration(25.0);
  gpik_req.ik_request.ik_link_name = "L7_wrist_roll_link";

  gpik_req.ik_request.pose_stamped.header.frame_id = "base_footprint";
  gpik_req.ik_request.pose_stamped.pose.position.x = 0.569;
  gpik_req.ik_request.pose_stamped.pose.position.y = 0.432;
  gpik_req.ik_request.pose_stamped.pose.position.z = 0.277;
