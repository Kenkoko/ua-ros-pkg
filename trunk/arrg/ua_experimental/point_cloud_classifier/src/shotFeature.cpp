...
//  shotFeature
//
//  Created by Mohsen Malmir on 9/29/11.
...
//argument processing
    if(argc<5)
    {
		...
		std::cout << "listFile is the name of file that contains the list of inpud pointcloud files" << endl;
	...
	while(!listFile.eof())
	{
		...
		if(argc>5)
			pcdFileFullName = pcdFilePath + "/" + pcdFileName;
		else
			pcdFileFullName = pcdFileName;
		//loading pcd file
