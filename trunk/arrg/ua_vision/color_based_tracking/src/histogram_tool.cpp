...
void update_words(CvHistogram* given_hist, string given_shape, std::vector<string> known, std::vector<string> unknown)
{
    ...
    {                    
        ...
        if( function == "make" ){
            FILE* histogram = fopen((unknown[i]+"_hist").c_str(),"wb");
    		fwrite(cvGetHistValue_3D(given_hist,0,0,0),sizeof(float),dims[0]*dims[1]*dims[2],histogram);
    		fclose(histogram);
		}
    }
...
void handleImage(const sensor_msgs::ImageConstPtr& msg_ptr) 
{
    ...
    else
    {
        ...
        cvThreshold(mono_image, mono_image, 20, 255, CV_THRESH_BINARY);
        ...
        if (function == "find" )
        {
            ...
            FILE* histogram = fopen((phrase+"_hist").c_str(),"rb");
     		fread(cvGetHistValue_3D(combined_hists,0,0,0),sizeof(float),dims[0]*dims[1]*dims[2],histogram);
      		fclose(histogram);
