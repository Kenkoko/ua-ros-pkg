...
//	camera_sub_ = n.subscribe(argv[2], 1, &Tracker::cameraCallback, this);
	...
	histogram = fopen(histogram_name.c_str(), "rb");
        if(histogram == NULL)
        {
	...
	fclose(histogram);
        ...
        /* this sets up the output stream */
	bound_pub_ = n_.advertise<geometry_msgs::PolygonStamped>(output_topic, 1);
...
/**/	}
	...
	if( show_images ) {
		cvShowImage( output_topic.c_str(), image );
