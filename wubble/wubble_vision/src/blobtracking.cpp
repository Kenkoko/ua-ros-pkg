...
int                              key;
int                              FrameNum;
CvVideoWriter*                   pFGAvi;
CvVideoWriter*                   pBTAvi;
...
//static int 
RunBlobTrackingAuto(ros::NodeHandle &n, char* image_stream1, CvBlobTrackerAuto* pTracker1,char* fgavi_name1 = NULL, char* btavi_name1 = NULL ) :
	n_(n), it_(n_)
{
    ...
    pFGAvi = NULL;
    pBTAvi = NULL;
    image_stream = image_stream1;
    pTracker = pTracker1;
    fgavi_name = fgavi_name1;
    btavi_name = btavi_name1;

    //cvNamedWindow( "FG", 0 );
    image_sub_ = it_.subscribe(
	image_stream, 1, &RunBlobTrackingAuto::imageCallback, this);
}
...
**/
	...
	try
	{
		pImg = bridge_.imgMsgToCv(msg_ptr, "bgr8");
	}
...
//    RunBlobTrackingAuto( pCap, pTracker, fgavi_name, btavi_name );
	ros::init(argc, argv, "RunBlobTrackingAuto");
	ros::NodeHandle n;
	RunBlobTrackingAuto ic(n,image_stream, pTracker, fgavi_name, btavi_name);
	ros::spin();
	return 0;
